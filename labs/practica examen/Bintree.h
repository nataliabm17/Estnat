...
class C_BinTree {
	private:
		...
		void eliminarHoja(S_node* paux){
			if(paux){
				if(paux->derecha==NULL&&paux->izquierda==NULL){
					borrar(paux->dato);
				}
				if(paux->izquierda){
					eliminarHoja(paux->izquierda);
				}
				if(paux->derecha){
					eliminarHoja(paux->derecha);
				}
			}
		}

		void borrarRango(S_node* paux, int min, int max){
			if(paux){
				if(paux->dato<max&&paux->dato>min){
					borrar(paux->dato);
				}
				if(paux->izquierda!=NULL){
					borrarRango(paux->izquierda,min,max);
				}
				if(paux->derecha!=NULL){
					borrarRango(paux->derecha,min,max);
				}
			}
		}
	...
	public:
		...
		void borrarRango(int min, int max){
			borrarRango(this->root,min,max);
		}

		void eliminarHoja(void){
			eliminarHoja(this->root);
		}
